...
from base.seismo_models import HistoBe13501,HistoBe13658, HistoBe13833, HistoBe13835, HistoBe13836,\
    ...
    HistoUm6378,HistoUm6379,HistoUm6380,HistoMp12626Slm,HistoUm10374Slm,SeismoWaveforms
...
def get_gage_data(gagelist,from_date,to_date,min_y,max_y,secondary):
    ...
    for index,gage_id in enumerate(gagelist):
        ...
        elif table == 'histo_um10374_slm':
            data = pd.DataFrame(list(HistoUm10374Slm.objects.using('seismo').filter(timestamp__range=[from_date, to_date]).values('timestamp',gage))).dropna()
        else: 
            data = None
...
def get_gage_datahz(gagelist,from_date,to_date,min_y,max_y,secondary):
    ...
    for index,gage_id in enumerate(gagelist):
        ...
        if table == 'histo_be13501':
            ...
            data = pd.DataFrame(list(HistoUm10374Slm.objects.using('seismo').filter(timestamp__range=[from_date, to_date]).values(freq,gage))).dropna()
        else: 
            data = None
